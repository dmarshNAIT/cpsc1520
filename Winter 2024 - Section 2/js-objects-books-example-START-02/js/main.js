...
const formElement = document.querySelector('#books-filter-form');
...
function renderBooks(library) /* library is our list of books */
{
    const tableBodyElement = document.querySelector('#book-rows');
...
function getAuthors(library) {
    // call map to create an array of authors
    let authors = library.map( (book) => { return book.author; } ); 

    // call filter to remove duplicates
    let uniqueAuthors = authors.filter( (author, index) => {
...
function renderAuthorOptions(library) {
  // get my list of authors
  let uniqueAuthors = getAuthors(library);

  let selectElement = document.querySelector('#author-select');

  // for each author, add it to the HTML of the appropriate element
...
function getFilteredBooks(searchTerm, propertyName, library) {
  // for each book in our library
  // return true if the book includes the search Term
...
formElement.addEventListener('submit', (event) => {
  ...
  // get the form values
  let titleSearch = event.target.elements['filter-query'].value.trim();
  let authorSearch = formElement.elements.author.value;

  // get the books
  let books = allBooks;

  // if a title has been provided, filter by title
  if(titleSearch.length !== 0) {
    books = getFilteredBooks(titleSearch, 'title', books);
  }
  // if an author has been provided, filter by author
  if(authorSearch.length !== 0) {
    books = getFilteredBooks(authorSearch, 'author', books);
  }
