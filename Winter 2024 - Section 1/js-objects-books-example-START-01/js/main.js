...
const authorElement = document.querySelector('#author-select');
let allBooks = getAllBooks(); // this is the list of all books 
...
function renderBooks(books) {
  ...
  /* for each element in books */
  books.forEach( (book) => {
    /* create the HTML template, and add it to the appropriate element */
    tableBodyElement.innerHTML += `
      <tr>
        <td>${book.bookId}</td>
        <td>${book.title}</td>
...
function getAuthors(books) {
  // use map to get a list of authors
  let authors = books.map( (book) => {
    return book.author;
  });

  // use filter to get a list of distinct authors
...
function renderAuthorOptions(books) {
  // get the list of authors
  const authors = getAuthors(books);

  // for each author in the array, add it as an option
...
formElement.addEventListener('submit', (event)  => {
  ...
  // get the values from the elements in the form
  let titleQuery = formElement.elements['filter-query'].value.trim();
  let authorQuery = formElement.elements.author.value;
  let books = allBooks;

  // call getBooksFilter()
  // if title is provided, call getBooksFilter() for the title
  if(titleQuery !== '') {
    books = getBooksFilter(titleQuery, 'title', books);
  }
  // if author is provided, call getBooksFilter() for the author
  if(authorQuery !== '') {
    books = getBooksFilter(authorQuery, 'author', books);
  }
...
function getBooksFilter(searchTerm, propertyName, books) {
  let filteredBooks = books.filter( (book) => {
    // check whether the current book contains the query
