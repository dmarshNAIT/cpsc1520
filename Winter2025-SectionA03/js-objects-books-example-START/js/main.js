...
const renderBooks = (books) => {
  ...
  books.forEach((book) => {
    ...
    // alternatively, instead of forEach, we could use map here, or for, or for/of
    // PRACTICE CHALLENGE: try replacing forEach with one of these
  })
};

// we want to render all the books when the page loads:
...
const renderAuthorOptions = (books) => {
  // use the books array to get the list of authors
  const authors = getAuthors(books);

  const dropdownElement = document.querySelector('.form-select');
