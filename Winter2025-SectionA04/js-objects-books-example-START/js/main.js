...
form.addEventListener('submit', (event) => {
  ...
  let myFilteredBooks = getAllBooks();

  // if title has a value, search by title
  if (titleSearch.length > 0) {
    // or if (titleSearch !== '')
    // or if (titleSearch) because an empty string evaluates to false
    // call getFilteredBooks
    myFilteredBooks = getFilteredBooks(titleSearch, 'title', myFilteredBooks);
  }

  // if author has a value, search by author
  if (authorSearch) {
    myFilteredBooks = getFilteredBooks(authorSearch, 'author', myFilteredBooks);
  }

  // render the filtered books
  renderBooks(myFilteredBooks);
});
